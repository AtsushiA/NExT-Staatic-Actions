...
Staatic の管理メニュー配下「スケジュール公開」専用ページ（Actionsとは別ページ）で、以下の頻度を選んで
...
**このページのみ編集者（Editor）以上の権限で操作できます。** メール・Cloudflare・Webhookの設定を含む
「Actions」ページは引き続き管理者（`manage_options`）のみがアクセスできます。編集者はスケジュールの
値以外の設定を閲覧・変更することはできません（保存時、それ以外の設定値はデータベースに保存されている
現在の値がそのまま維持されます）。

編集者への操作許可自体は、管理者が Actions → 詳細設定タブのチェックボックスでON/OFFできます
（デフォルトは許可）。オフにすると編集者は次回のスケジュール公開ページへのアクセス時に権限エラーになります。
